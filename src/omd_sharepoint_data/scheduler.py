...
class SyncScheduler:
    ...
    async def _run_sync(self):
        ...
        try:
            results = await self.sync_manager.sync_pdf_files(sharepoint_config.base_folder_name)
    ...
    async def run_pdf_sync_now(self, target_folder_name: str = sharepoint_config.base_folder_name) -> Optional[Dict[str, int]]:
        """手动触发PDF增量同步操作。"""
        if not self.sync_manager:
        ...
        try:
            logger.info(f"运行手动PDF同步操作: {target_folder_name}")
            results = await self.sync_manager.sync_pdf_files(target_folder_name)
            logger.info(f"手动PDF同步完成: {results}")
            return results
        except Exception as e:
            logger.error(f"手动PDF同步失败: {e}")
            return None
